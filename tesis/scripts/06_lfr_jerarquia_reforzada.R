...
nodos_lfr <- sort(unique(micro_conocidas$node_id))

stopifnot(
  identical(
    nodos_lfr,
    sort(unique(macro_conocidas$node_id))
  )
)

vertices <- data.frame(
  name = as.character(nodos_lfr)
)
...
# Validaciones
n_micro_conocidas <- length(
  unique(micro_conocidas$micro_comunidad)
)

n_macro_conocidas <- length(
  unique(macro_conocidas$macro_comunidad)
)

stopifnot(vcount(g_lfr) == 5000)
stopifnot(ecount(g_lfr) == 48631)
stopifnot(nrow(micro_conocidas) == 5000)
stopifnot(nrow(macro_conocidas) == 5000)
stopifnot(n_micro_conocidas == 147)
stopifnot(n_macro_conocidas == 6)
...
stopifnot(
  nrow(comparacion_lfr) == length(nodos_lfr)
)
...
resumen_lfr <- c(
  ...
  paste("Macrocomunidades conocidas:", n_macro_conocidas),
  paste("Microcomunidades conocidas:", n_micro_conocidas),
  "",
